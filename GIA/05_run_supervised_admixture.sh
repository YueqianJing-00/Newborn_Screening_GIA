...
if [[ $DRY_RUN != 1 ]]; then
  command -v "$ADMIXTURE" >/dev/null 2>&1 || fail "required command not found: $ADMIXTURE"
  for extension in bed bim fam; do
    [[ -f ${JOINT_BFILE}.${extension} ]] || fail "missing PLINK file: ${JOINT_BFILE}.${extension}"
  done
  [[ -f $JOINT_BFILE.pop ]] || fail "missing file: $JOINT_BFILE.pop"

  fam_rows=$(wc -l <"$JOINT_BFILE.fam")
  pop_rows=$(wc -l <"$JOINT_BFILE.pop")
  [[ $fam_rows -eq $pop_rows ]] || fail "joint FAM and POP row counts differ ($fam_rows vs $pop_rows)"
fi
