...
```bash
cp workflow/ancestry/config.example.env workflow/ancestry/config.env
set -a
source workflow/ancestry/config.env
set +a
```
...
WORK_DIR="$GLOBAL_REFERENCE_WORK_DIR" \
  bash workflow/ancestry/global/01_prepare_1000g_reference.sh
```
...
```bash
Rscript workflow/ancestry/global/02_select_reference_samples.R \
...
WORK_DIR="$GLOBAL_JOINT_WORK_DIR" \
  bash workflow/ancestry/global/03_prepare_joint_dataset.sh

Rscript workflow/ancestry/global/04_build_supervised_pop.R \
...
JOINT_BFILE="$GLOBAL_JOINT_WORK_DIR/joint" \
  bash workflow/ancestry/global/05_run_supervised_admixture.sh
```
