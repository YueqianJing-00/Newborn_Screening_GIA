...
# Plot Figure 1C: PRE-GIA cross-classification in the full cohort.
...
# Restore empty combinations so every PRE and GIA level is shown.
counts <- read.csv(file.path(source_dir, "figure1_cross_classification_all.csv"))
plot_data <- expand_grid(
  ...
  mutate(
    ...
    label = ifelse(n == 0, "", n),
    text_color = ifelse(n >= 0.35 * max(n), "white", "black")
  )
...
panel <- ggplot(plot_data, aes(largest_gia_component, assigned_pre_plot, fill = n)) +
  ...
  labs(x = "Largest GIA component (all individuals)", y = "Assigned PRE") +
  theme_panel(8.5) +
  theme(panel.border = element_rect(color = "grey35", fill = NA, linewidth = 0.45))

save_panel(panel, figure_dir, "Figure1C_concordance_all", 5.2, 4.3)
